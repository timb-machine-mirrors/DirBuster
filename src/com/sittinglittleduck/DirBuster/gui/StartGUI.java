...
public class StartGUI extends javax.swing.JFrame
{
    ...
    public StartGUI()
    {
        try
        {
            ...
            jPanelSetup.jCheckBoxScanDefault.setSelected(manager.getEnableNiktoByDefault());
            jPanelSetup.jCheckBoxRecursive.setSelected(manager.getEnableRecursiveByDefault());
    ...
    private void initComponents() {
        ...
        java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
        setBounds((screenSize.width-851)/2, (screenSize.height-547)/2, 851, 547);
    ...
    public void setPrefferedFrameSize()
    {

        java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
        //set the size of the app based on the setup panel
        int width = jPanelSetup.getPreferredSize().width + 20;

        //total height = setup + status + menubar + estimate for taskbar
        int height = jPanelSetup.getPreferredSize().height + jPanelStatus.getPreferredSize().height + jMenuBar.getPreferredSize().height + 20;
    ...
    private void updateAllComponentTreesUI()
    {

        //must force update on every component not just those currently visible in the main frame tree.
        SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jMenuBar.getParent());
        SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jPanelReport);
        SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jPanelRunning);
        SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jPanelSetup);
        SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jPanelSetup.jPanelBruteForcedOptions);
                        ...
                        SwingUtilities.invokeLater(new Runnable()
                        {
                            ...
                            public void run()
                            {
                                try
                                {
                                    ...
                                    //SwingUtilities.updateComponentTreeUI(Manager.getInstance().gui.jMenuBar.getParent());
                                    Manager.getInstance().gui.updateAllComponentTreesUI();
                                    setPrefferedFrameSize();
                                }
                                catch(ClassNotFoundException ex)
                                {
