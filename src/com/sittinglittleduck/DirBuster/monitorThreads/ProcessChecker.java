    ...
    public void run()
    {
        ...
        if(timePassed > 0)
        {
            ...
            if(!manager.isHeadLessMode())
            {
                ...
                /*
                 * updates all the compoenets for the multi threaded work gen
                 */
                if(manager.getScanType() == Manager.LIST_BASED)
                {
                    for(int a = 0; a < manager.genThreads.size(); a++)
                    {
                        int complete = manager.genThreads.elementAt(a).getCurrentPoint();
                        JPanelScanInfo tempInfo = (JPanelScanInfo) manager.gui.jPanelRunning.jPanelScanInfoBase.getComponent(a);
                        tempInfo.setProgressCurrentValue(complete);

                    }
                }
                else if(manager.getScanType() == Manager.BRUTE_BASED)
                {
                    for(int a = 0; a < manager.genThreads.size(); a++)
                    {
                        int complete = manager.genThreads.elementAt(a).getCurrentPoint();
                ...
                else if(manager.getScanType() == Manager.LIST_BASED_FUZZ)
                {
                    JPanelScanInfo tempInfo = (JPanelScanInfo) manager.gui.jPanelRunning.jPanelScanInfoBase.getComponent(0);
                    tempInfo.setProgressCurrentValue(manager.workGenFuzz.getCurrentCount());
                }
                else if(manager.getScanType() == Manager.BRUTE_BASED_FUZZ)
                {
                    JPanelScanInfo tempInfo = (JPanelScanInfo) manager.gui.jPanelRunning.jPanelScanInfoBase.getComponent(0);
                    tempInfo.setProgressCurrentValue(manager.workGenBruteFuzz.getCurrentDone());
                }

                /*
                 * update the title of the tab to show how many items we have found
                 */

                manager.gui.jPanelRunning.jTabbedPaneViewResults.setTitleAt(1, "Results - List View: Dirs: " + manager.getTotalDirsFound() + " Files: " + manager.getTotalFilesFound());

                /*
                 * gives feed back on how many tasks are currently running
                 */
                ...
                for(int a = 0; a < manager.genThreads.size(); a++)
                {
                    ...
                    else if(manager.genThreads.elementAt(a).isWorking() && manager.genThreads.elementAt(a).isPaused())
                    {
                        paused++;
                    }
                    else
                ...
                manager.gui.jPanelRunning.jTabbedPaneViewResults.setTitleAt(0, "Scan Info: R: " + running + " P: " + paused + " C: " + complete);
