...
import com.sittinglittleduck.DirBuster.workGenerators.WorkerGeneratorNiktoMultiThreaded;
...
public class Manager implements ProcessChecker.ProcessUpdate
{
    ...
    /*
     * Stores the location of the nikto file, which is used for finding default files
     */
    private String niktoFileLocation;
    ...
    /*
     * stores the flag for performing scanning of default files based on the nikto database files
     */
    private boolean defaultFileScan = false;
    ...
    public void start()
    {
        try
        {
            ...
            /*
             * create the nikto work threads, if we need to
             */
            if (defaultFileScan)
            {
                WorkerGeneratorNiktoMultiThreaded niktotmp = new WorkerGeneratorNiktoMultiThreaded(startPoint);
                genThreads.addElement(niktotmp);

                if (!headLessMode)
                {
                    gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(genThreads.size() - 1, Manager.LIST_BASED, niktotmp.numberOfRequestsToSend()));
                }
            }
            ...
            if (pureBrute)
            {
                ...
                if (doDirs)
                {
                    ...
                    gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(genThreads.size() - 1, Manager.BRUTE_BASED, (int) manager.getTotalPass()));
                }
            ...
            else if (urlFuzz)
            {
                ...
                gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(-1, Manager.LIST_BASED_FUZZ, (int) manager.getTotalPass()));
                workGenThread.start();
            ...
            else if (pureBrutefuzz)
            {
                ...
                gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(-1, Manager.BRUTE_BASED_FUZZ, (int) manager.getTotalPass()));
                workGenThread.start();
            }
            else
            {
                /*
                 * if we are actually scanning the dir or files
                 */
                if (doFiles || doDirs)
                {
                    /*
                    ...
                    if (doDirs)
                    {
                        ...
                        gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(genThreads.size() - 1, Manager.LIST_BASED, (int) manager.getTotalPass()));
                    }
    ...
    public synchronized void foundDir(URL url, int statusCode, String responseHeader, String responseBody, BaseCase baseCaseObj)
    {
        ...
        if (!isStartPoint && isRecursive() && (manager.scanType == Manager.LIST_BASED || manager.scanType == Manager.BRUTE_BASED))
        {
            ...
            if (doDirs)
            {
                ...
                if (!headLessMode)
                {
                    gui.jPanelRunning.addScanInfoObject(new JPanelScanInfo(genThreads.size() - 1, manager.scanType, (int) manager.getTotalPass()));
                }
        ...
        /*
         * update the counter of how many we have found
         */
        totalDirsFound++;
    ...
    public void youAreFinished()
    {
        ...
        /*
         * stop all the work generator threads
         */
        for (int a = 0; a < genThreads.size(); a++)
        {
            genThreads.elementAt(a).stopMe();
        }
    ...
    public void loadPrefs()
    {
        ...
        niktoFileLocation = userPrefs.get("NiktoFileLocation", System.getProperty("user.dir") + File.separatorChar + "niktoFiles");
    ...
    public String getNiktoFileLocation()
    {
        return niktoFileLocation;
    }

    public void setNiktoFileLocation(String niktoFileLocation)
    {
        this.niktoFileLocation = niktoFileLocation;
        userPrefs.put("NiktoFileLocation", niktoFileLocation);
    }

    public boolean getEnableNiktoByDefault()
    {
        return userPrefs.getBoolean("NiktoByDefault", false);
    }

    public void setEnableNiktoByDefault(boolean enableByDefault)
    {
        userPrefs.putBoolean("NiktoByDefault", enableByDefault);
    }

    public void setPerformDefaultFileScan(boolean run)
    {
        defaultFileScan = run;
    }

    public boolean getEnableRecursiveByDefault()
    {
        return userPrefs.getBoolean("RecursiveByDefault", true);
    }

    public void setEnableRecursiveByDefault(boolean enableByDefault)
    {
        userPrefs.putBoolean("RecursiveByDefault", enableByDefault);
    }
}
